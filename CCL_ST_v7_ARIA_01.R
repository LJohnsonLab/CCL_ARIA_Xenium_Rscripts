...
annotation_list <- map(files, function(path) {
  ...
  df %>%
    mutate(
      ...
      Sample_ID = sample_id,
      anatomical_region = region # CANNOT NAME METADATA COLUMN "region" bc it is conserved in sdata
    )
})
...
merged.obj <- AddMetaData(merged.obj, metadata = annotation_df_unique["anatomical_region"]) # changed it from "Region" bc of squidpy analyses
